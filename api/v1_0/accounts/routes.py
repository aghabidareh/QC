...
REQUIRED_SCOPES = {
    "/wallet": ["customer.wallet.read"],
    "/wallet/spend": ["customer.wallet.write"],
}
...
@account_router.get("/wallet")
async def get_wallet(request: Request, current_user: Dict = Depends(get_current_user)):
    scopes = current_user.get("scopes", [])
    if "customer.wallet.read" not in scopes:
        logger.warning(f"Insufficient scopes for user {current_user['user_id']}: {scopes}")
        raise HTTPException(status_code=403, detail="Insufficient scopes")

    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{KONG_GATEWAY_URL}/wallet",
                headers={
                    "Authorization": f"Bearer {current_user['access_token']}",
                    "X-Kong-Auth-Client-Id": current_user["client_id"],
                },
            )
            if response.status_code != 200:
                logger.error(f"Wallet request failed: {response.text}")
                raise HTTPException(status_code=response.status_code, detail="Failed to access wallet")

            logger.info(f"User {current_user['user_id']} accessed wallet via client {current_user['client_id']}")
            return response.json()
    except httpx.HTTPError:
        logger.error("HTTP error during wallet request")
        raise HTTPException(status_code=500, detail="Failed to communicate with resource server")


@account_router.post("/wallet/spend")
async def spend_wallet(amount: float, request: Request, current_user: Dict = Depends(get_current_user)):
    scopes = current_user.get("scopes", [])
    if "customer.wallet.write" not in scopes:
        logger.warning(f"Insufficient scopes for user {current_user['user_id']}: {scopes}")
        raise HTTPException(status_code=403, detail="Insufficient scopes")
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{KONG_GATEWAY_URL}/wallet/spend",
                json={"amount": amount},
                headers={
                    "Authorization": f"Bearer {current_user['access_token']}",
                    "X-Kong-Auth-Client-Id": current_user["client_id"],
            ...
            if response.status_code != 200:
                logger.error(f"Wallet spend failed: {response.text}")
                raise HTTPException(status_code=response.status_code, detail="Failed to spend from wallet")

            logger.info(
                f"User {current_user['user_id']} spent {amount} from wallet via client {current_user['client_id']}")
            return response.json()
    except httpx.HTTPError:
        logger.error("HTTP error during wallet spend")
        raise HTTPException(status_code=500, detail="Failed to communicate with resource server")
